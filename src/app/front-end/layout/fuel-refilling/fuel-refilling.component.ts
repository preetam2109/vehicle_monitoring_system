...
import { tbl_fuel_refilling, tbl_travel, tbl_vehicle, tbl_vehicle_type } from 'src/app/model/tbl-model';

@Component({
  selector: 'app-fuel-refilling',
  templateUrl: './fuel-refilling.component.html',
  styleUrls: ['./fuel-refilling.component.css']
})
export class FuelRefillingComponent implements OnInit {
  api_controller_name= 'fuel_refilling'; 
  res_data: any;
  res_data_filter: tbl_fuel_refilling[] = [];
  base: any; 
  dt: any;
  ...
  public onEdit(row:any)
  {
    ...
    this.router.navigate(
      ['/create-fuel-refilling'],
      { queryParams: { id: row.id,vid:row.vehicle_id,rd:row.refilling_date,qty:row.quantity,ft:row.fuel_station,sa:row.station_address} }
    );
  }
